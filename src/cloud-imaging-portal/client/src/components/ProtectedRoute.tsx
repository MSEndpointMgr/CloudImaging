...
export function ProtectedRoute({ children }: Props): React.ReactElement | null {
  ...
  // Only the portal API scope is required here. The optional Graph profile-photo scope
  // (see AuthProvider) is deliberately requested separately, after sign-in, so a tenant
  // that blocks user consent to Microsoft Graph can never break this mandatory sign-in.
