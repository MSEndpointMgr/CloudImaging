...
- [ ] T040a [US2] Implement Operator API single-session image assign endpoint (POST /api/sessions/{sessionId}/assign) with CloudImaging.PortalAccess role enforcement and proxy call to Imaging Core API assign endpoint over Private Link in src/CloudImaging.OperatorApi/Functions/AssignSessionFunction.cs
