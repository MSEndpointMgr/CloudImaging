...
- USB bootable partition minimum size: 2 GB; cache partition minimum: 24 GB (holds one OS image at the 20 GB supported maximum plus filesystem overhead)
...
| Staged direct-to-blob upload for OS images and boot images | Avoids routing large binary files (up to 20 GB) through Portal backend and Operator API; browser/tool uploads directly to Storage via short-lived SAS token URL | API-proxied upload would impose network cost and memory pressure on App Service and Function Apps for large image files |
