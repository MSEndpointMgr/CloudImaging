...
- **Upload format**: a `.zip` (or folder tree staged the same chunked-upload way as WIM uploads)
  ...
  concept doesn't map cleanly, so it needs its own validation service rather than reusing
...
- **Portal UI**: new `DriverPackagesPage.tsx` mirroring `BootImagesPage.tsx`/`RecoveryImagesPage.tsx`
  ...
  single-entry concept needed, since multiple active entries are the normal state.
...
1. **Do it on-device instead, at apply time** (before `/Apply-Image`, targeting the *downloaded
   ...
   afterward (the practical difference is minimal for storage/NIC/chipset drivers, since both are
...
requirement surfaces for a single golden WIM per model. If so, prefer option 1 (on-device,
